...
import 'package:meeting_schedule/LeadingSpace.dart';
...
final _formKeyforMeet = GlobalKey<FormState>();
var fff;
bool isSwitched = false;
var textValue = 'Switch is OFF';

enum SelectionPeriods { Daily, Weekly, Monthly }
...
class AddmeetingWidget extends StatefulWidget {
  ...
  const AddmeetingWidget({Key? key,  required this.PrimaryColor,this.AppbarWidget,
    required this.CancelButtonText,required this.CancelButtonOnTap,
    required this.SubmitButtonText,}) : super(key: key);
  @override
...
class _AddmeetingWidgetState extends State<AddmeetingWidget> {
  ...
  SelectionPeriods _site = SelectionPeriods.Daily;
  ...
  Widget build(BuildContext context) {
    ...
    return   Scaffold(
        appBar:widget.AppbarWidget ==null ? AppBar(title: Text('Schedule Meeting'),):widget.AppbarWidget,
  ...
  ScheduleMeetOntap( )  async{
    ...
    if(_formKeyforMeet.currentState!.validate()) {
        ...
        if((_endController.text).length>2){
          if(!isSwitched){
            finalresultjson=jsonEncode({
              "m_purpose": _fnameController.text,

              "m_teamname": "TBD",
              "m_timestampfrom": _startController.text,
              "m_timestampto": _endController.text,
              "m_place": _pfnameController.text,
              "m_notes": _plnameController.text,
              "m_timestamp":starttime,
              "m_end_hour":hrtime,
              "m_end_minute":mintime,
              "repeat":isSwitched ,
              if(isSwitched)
                "selected_days":daysofrecc,
              if(isSwitched)
                "repeatfor":qytmonthly,
